...
class GoLingo {
    ...
    ignoredTags: string[] =
        ['iframe', 'img', 'style', 'script', 'meta', 'link', 'head', 'html', 'noscript', 'audio', 'video', 'object', 'canvas', 'svg', 'map', 'area', 'embed', 'param', 'track', 'source'];
    ...
    async loadData() {
        ...
        const response = await fetch(`${GOLINGO_HOST}api/tr?code=${this.code}&path=${path}`);
        ...
        for (let key of Object.keys(translates)) {
            ...
            translates[key].texts = translates[key].texts.filter((v, i, a) => a.findIndex(t => (t.wrapperId === v.wrapperId)) === i);
        }
    ...
    async startTranslate() {
        ...
        this.detectAllTexts();
        console.log("## GoLingo: startTranslate", this.texts, this.translates[path], this.currentLang, this.translatedTo);

        if (this.translates?.[path]?.[this.currentLang as string] &&
                ...
                if (type) {
                    switch (type) {
                        case "sync-translations":
                            ...
                            this.setCurrentLang(
                                data.data.currentLang
                            );
                            this.mainLang = data.data.mainLang;
                            if (this.mainLang && this.translates?.[path]?.[this.mainLang] != null) {
                                this.detectAllTexts();
                                this.overrideTexts();
                            }
                            break;
    ...
    toggleTextItem(wrapperId: string, disabled: boolean) {
        ...
        document.getElementById(wrapperId)?.classList.toggle("golingo_disabled", disabled);
    }
    // syncClicked(item: 
    syncTexts = () => {
    ...
    private overrideTexts() {
        ...
        texts.forEach((text) => {
            ...
            if (wrapper && text.text != wrapper.textContent) {
                console.log("## GoLingo: overrideTexts", text.text, " with ", wrapper.textContent, text.text == wrapper.textContent);
    ...
    private detectTexts(node: Node, excludedTags: string[]): TextItem[] {
        ...
        if (node.nodeType === 3) {  // Text node and not already highlighted
            // @ts-ignore
            ...
            if (content !== '') {
                // @ts-ignore
                const isElExists = node.parentNode.classList.contains('golingo_highlighted');
            ...
            // 
        } else if (node.nodeType === 1 && !excludedTags.includes(tagName)) {  // Element node
            const elementNode = node as HTMLElement;
            for (let i = 0; i < elementNode.childNodes.length; i++) {
                let detectedNodes = this.detectTexts(elementNode.childNodes[i], excludedTags);
    ...
    private observeDOMChanges(): void {
        const observer: MutationObserver = new MutationObserver((mutationsList, observer) => {
    ...
    public addBasicLanguageSelector() {
        ...
        this.findCurrentLang();
        console.log("## GoLingo: addBasicLanguageSelector", this.currentLang, langs, this.translates[path]);
        ...
        const wrapper = html`${() => {
            ...
            return html`<div class="${className}">
                ...
                ${() => {
                    return html`<select class="golingo-select-lang" @change="${(e: any) => { golingo.switchLanguage(e.target.value) }}">
                    ${langs.map((lang) => {
                        return html`<option value="${lang}" selected="${() => lang === this.reactiveData.currentLang}">${getLangName(lang)}</option>`
                    }
                    )}
                </select>`
                }}
...
type GolingoEvents = "language-change" | "translate" | "path-change" | "init" | "translates-loaded";
