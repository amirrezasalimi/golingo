...
import AsyncTalk, { getInstanceReturn } from "../../../../shared/async-talk";
...
const Site = () => {
  ...
  const userSelectedLangs = useMemo(() => {
    return (
      site.data.data?.languages?.filter((lang) => lang != site.mainLang) ?? []
    );
  }, [site.data.data]);

  const selectedLangs = [site.mainLang, ...(userSelectedLangs ?? [])] ?? [];
  ...
  const selectedPagewithoutSlash = useMemo(() => {
    ...
    return site.selectedPage.startsWith("/")
      ? site.selectedPage.slice(1, site.selectedPage.length)
      : site.selectedPage;
  }, [site.selectedPage]);
  const currentUrl = `${fullUrl}/${
    selectedPagewithoutSlash === "/" ? "" : selectedPagewithoutSlash
  }?golingoDev=1`;
  ...
  const currentPage = site.pages.find(
    (page) => page.path === site.selectedPage
  );

  const iframeRef = useRef<HTMLIFrameElement>(null);

  const asyncPostMessage = useMemo(() => {
    if (!iframeRef.current) return null;
    return AsyncTalk.getInstance<DevMethodsPromises, PanelMethodsPromises>(
  ...
  const syncTextsRef = useRef<(data: TextItem[]) => void | null>(null);
  const syncTextsRefValue = (data: TextItem[]) => {
    ...
    const isChanged = data.some((item) => {
      const oldText = oldTranslates[site.mainLang]?.texts?.find(
        (text) =>
          text.wrapperId === item.wrapperId && text.position === item.position
      )?.text;
  ...
  // @ts-ignore
  syncTextsRef.current = syncTextsRefValue;
  ...
  const postMessage = (type: keyof DevMethodsPromises, data: any = {}) => {
    return new Promise((resolve) => {
      console.log("## GoLingo: postMessage", 1);

      if (!asyncPostMessage) return;
      asyncPostMessage?.send(type, data).then((res) => {
        resolve(res);
      });
    });
  };
  ...
  useEffect(() => {
    if (!asyncPostMessage) return;
    const unsubscribe = asyncPostMessage?.listen(
      // @ts-ignore
      async (request) => {
        console.log(`## GoLingo: panelMethod`, request);
        const { uid, functionName, args } = request;
        switch (functionName) {
          case "syncLinks":
            let links = args as string[];

            syncLinks(links);
            break;
          case "syncTexts":
            const data = args as TextItem[];
            console.log(
              ` GoLingo: syncTexts Panel | currentPage:`,
              currentPage
            );
            syncTextsRef?.current?.(data);
            break;
        }
  ...
  const selectPage = (page: any) => {
    ...
    toggleLangView(site.mainLang);
  };
  const currentTexts = useMemo(() => {
    return (
      (
        currentPage?.translates?.[site.mainLang as string] ?? {
          texts: [],
        }
      )?.texts ?? []
    );
  ...
  const translatedTexts = useMemo(() => {
    return (
      (
        currentPage?.translates?.[site.currentLangView as string] ?? {
          texts: [],
        }
      )?.texts ?? []
    );
  ...
  const currentLangFull =
    site.languages?.[site.currentLangView as keyof typeof site.languages]
      ?.native ?? "";
  ...
  useEffect(() => {
    postMessage("syncTranslations", {
      currentLang: site.currentLangView,
      mainLang: site.mainLang,
      translations: currentPage?.translates ?? {},
    }).then(() => {
      console.log("## GoLingo: syncTranslations", 1);

      let lang = site.currentLangView as string;
      const translates = currentPage?.translates ?? {};
      if (!translates?.[lang]) {
        lang = site.mainLang;
      }
      postMessage("overrideTexts", lang);
    });
  }, [currentPage, site.currentLangView]);
  ...
  >([]);
  const [autoPageModalIsOpen, setAutoPageModalIsOpen] =
    useState<boolean>(false);

  const openAutoLinkModal = () => {
    postMessage("identifyLinks");
  ...
  const autoPageModalAddAction = async () => {
    ...
    let addedPages = 0;
    const selectedPages = autoPageItems
    ...
    toast.success(
      `${addedPages} of ${selectedPages.length} pages added successfully`
    );
  };
  ...
  return (
    <>
      <Modal
        size="lg"
        isOpen={autoPageModalIsOpen}
        onClose={() => setAutoPageModalIsOpen(false)}
      >
        <ModalContent className="w-[60vw]">
          <ModalHeader>Auto Detect Pages</ModalHeader>
          <ModalBody className="flex flex-col gap-4">
            <div className="flex flex-col gap-2 items-center">
              {autoPageItemsUnique.length > 0 ? (
                <>
                  <div>
                    this is all links we found in your website, you can enable
                    or disable them
                  </div>
                  ...
                  <Button
                    ...
                    onClick={autoPageModalAddAction}
                    className="mt-4"
                  >
                    Add
                  </Button>
      ...
      >
        <ModalContent className="w-[60vw]">
          <ModalHeader>Languages</ModalHeader>
          <ModalBody className="flex flex-col gap-4">
            <div>
              <Input
                value={langFilter}
              ...
              {Object.keys(allLanguages)
                ...
                .map((lang, i) => {
                  return (
                    <div
                      key={i}
                      className="group flex justify-between items-center rounded-md px-2 py-2 gap-2 border border-gray-300"
                    >
                      <div className="flex gap-2 items-center">
                        ...
                        <span>
                          {
                            allLanguages[lang as keyof typeof allLanguages]
                              .native
                          }
                        </span>
                      </div>
                      <div className="flex gap-2 items-center">
                        {lang == site.mainLang ? (
                          ...
                          <div className="group-hover:opacity-100 opacity-0 cursor-none group-hover:cursor-default">
                            <Button
                              ...
                              isLoading={
                                site.updateWebsite.isPending &&
                                site.currentChangeLang === lang
                              }
                        ...
                        {lang !== site.mainLang && (
                          <Checkbox
                            ...
                            isSelected={site.isLangActive(lang)}
                            onValueChange={(checked) =>
                              site.toggleLang(lang, checked)
                            }
                          />
                        )}
                      </div>
      ...
      <div className="flex flex-col items-start w-full">
        <div className="flex justify-between items-center gap-2">
          <h1 className="text-2xl font-bold">
            {site.data.isLoading ? <CircularProgress size="sm" /> : url}
          </h1>
          {
            // verify
            site.data.isFetched &&
        ...
        {site.data.isFetched && (
          <>
            {/* verify site */}
            {site.checkVerify.isFetched && site.checkVerify.data === false && (
                ...
                <CardBody className="flex flex-col w-full gap-4">
                          ...
                          >
                            ...
                            {page.path !== "/" && (
                              <Button
                                ...
                                variant="light"
                                isLoading={
                                  site.deletePage.isPending &&
                                  site.deletePage.variables?.id === page.id
                                }
                                disabled={
                                  site.deletePage.isPending &&
                                  site.deletePage.variables?.id === page.id
                                ...
                                onClick={() =>
                                  site.deletePage.mutateAsync({ id: page.id })
                                }
                              >
                    ...
                    {selectedLangs.map((lang, i) => {
                      return (
                        <Button
                          ...
                          onClick={() => toggleLangView(lang)}
                        >
                          {
                            site.languages[lang as keyof typeof site.languages]
                              .native
                          }
                          {lang == site.mainLang && <span>(Main)</span>}
                        </Button>
                      );
                    })}
                  </div>
                  ...
                  <div className="mt-4">
                    <div className="flex flex-col gap-2 p-3 bg-gray-100 rounded-md">
                      <span>Current Page: {site.selectedPage}</span>
                      <div className="flex flex-col gap-2">
                        ...
                        <div>
                          ...
                          {
                            site.languages[
                              site.mainLang as keyof typeof site.languages
                            ].native
                          }
                        </div>
                        <div>To Language: {currentLangFull}</div>
                        <div>
                          {/* detected texts */}
                          ...
                          <>
                            ...
                            <div className="flex w-[200px] flex-col gap-2">
                              ...
                              <Button
                                ...
                                onClick={clearCurrentTranslations}
                              >
                                Clear Translations
                              </Button>
                            </div>
                      ...
                      <div className="flex  justify-center w-full h-full relative">
                        {isPageReady && (
                          <iframe
                            ...
                            style={{
                              width:
                                selectedDevice === "desktop" ? "100%" : "375px",
                              height:
